...
# Cleaning the content of the dataframes
def clean_content(df_dirty):
    ...
    for col in ['n3k_title', 'n3k_body', 'bs_title', 'bs_body', 'bs_paragraph']:
    ...
    # Doing the same for bs_title, bs_body, and bs_paragraph
    ...
    for word in ['javascript', 'copyright', 'benzinga', 'djreprints']:
        df_dirty.loc[(df_dirty['bs_paragraph'].str.lower().str.contains(word))
                    & (df_dirty['bs_paragraph'].str.len() < 400), 'bs_paragraph'] = ''

    # replace special characters with nan for paragraph column
    df_dirty["paragraph"] = df_dirty["bs_paragraph"].replace(r'^\s*$', np.nan, regex=True)
